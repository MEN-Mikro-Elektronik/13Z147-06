 ...
 *       \author Apatil
 *        $Date: 2016/04/27 13:47:47 $
 *    $Revision: 1.1 $
 *
 *       \brief  Z147 test tool for timing (rx part)
 *
...
/*-------------------------------[ History ]--------------------------------
 *
 * $Log: timing_test_rx_part.c,v $
 * Revision 1.1  2016/04/27 13:47:47  Apatil
 * Initial Revision
 *
 *---------------------------------------------------------------------------
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2 || strcmp(argv[1],"-?")==0) {
		printf("Syntax: z147_timing_test_rx <rxDevice>\n");
		printf("Function: Z147 test tool for timing (rx part)\n");
		return(1);
	}
...
static void __MAPILIB SignalHandler( u_int32 sig )
{
	...
	u_int32 currentTime = 0;

	if( sig == UOS_SIG_USR1 ) {
		++G_sigCount;
		currentTime = UOS_MsecTimerGet();
